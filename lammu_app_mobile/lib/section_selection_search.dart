...
class SectionSelectionSearch extends StatefulWidget {
  const SectionSelectionSearch({super.key});
  @override
  State<SectionSelectionSearch> createState() => _SectionSelectionSearchState();
}

class _SectionSelectionSearchState extends State<SectionSelectionSearch> {
  String selectedYear = '';
  ...
  Future searchStudent() async {
    if (this.yearValue == "Ninguno" ||
      ...
      final route = MaterialPageRoute(
          builder: (context) => const StudentInformationView());
        ...
        body: SafeArea(
            child: Center(
              ...
              Center(
                child: Center(
                  child: Text(
                    " Ingrese los datos del estudiante del que desea buscar información:",
...
void _MissingStudentErrorDialog(BuildContext context) {
  AlertDialog alert = const AlertDialog(
    title: Text(
      "No encontramos el/la estudiante en esta sección :(",
      style: TextStyle(
          fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white),
    ),
    backgroundColor: Colors.red,
